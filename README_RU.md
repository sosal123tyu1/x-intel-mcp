...
`x-intel-mcp` — легковесный локальный MCP-сервер, работающий через `stdio`. Он подключает вашего AI-ассистента к открытым данным X напрямую без авторизации.

---

## ✨ Возможности

- 🆓 **0 рублей за API** — работает напрямую через открытые syndication-эндпоинты.
